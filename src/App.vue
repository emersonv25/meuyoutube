  ...
  created: function () {
    this.$http.interceptors.response.use(undefined, function (err) {
      return new Promise(function () {
        if ( err.response.status == 400 /*|| err.response.status == 400 || err.response.status === 404 || err.response.status == 404 && err.response.config && !err.response.config.__isRetryRequest*/) {
